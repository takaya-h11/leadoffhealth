...
export async function approveAppointment(appointmentId: string, slotId: string) {
  ...
  try {
    ...
    if (appointmentError) {
      ...
      redirect('/therapist/appointments?message=Appointment+approval+failed')
    }
...
export async function rejectAppointment(
  ...
  if (!rejectedReason || rejectedReason.trim().length === 0) {
    redirect('/therapist/appointments?message=Rejected+reason+is+required')
  }
  ...
  try {
    ...
    if (appointmentError) {
      ...
      redirect('/therapist/appointments?message=Appointment+rejection+failed')
    }
        ...
        if (appointmentInfo.requested_by) {
          ...
          await createNotification(
            ...
            `${startTime.toLocaleDateString('ja-JP', {
              month: 'long',
              day: 'numeric',
            })}の予約が拒否されました。理由: ${appointmentInfo.rejected_reason || '記載なし'}`,
            appointmentId
          )
        }
      }
    ...
    revalidatePath('/therapist/appointments')
    revalidatePath('/therapist/schedule')
    redirect('/therapist/appointments?message=Appointment+rejected+successfully')
  } catch (error) {
    // redirect() throws NEXT_REDIRECT - must re-throw it
    if (error instanceof Error && error.message === 'NEXT_REDIRECT') {
      throw error
