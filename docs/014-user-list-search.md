...
## 概要
管理者が全ユーザー（管理者・整体師・法人担当者）を一覧表示し、検索・フィルタリングできる機能を実装する。

## 前提条件
- ✅ 認証機能が実装されている（001-010完了）
- ✅ 法人管理機能が実装されている（011完了）
- ✅ 整体師管理機能が実装されている（012完了）
- ✅ 法人担当者登録機能が実装されている（013完了）
- ✅ `users`, `companies`, `therapists`テーブルが作成されている

## タスク

### 1. ユーザー一覧ページ作成
- [ ] `src/app/(protected)/admin/users`ディレクトリを作成
- [ ] `src/app/(protected)/admin/users/page.tsx`を作成
- [ ] 管理者権限チェックを実装
- [ ] 全ユーザー一覧を表示
...
## 完了条件
- [ ] 全ユーザー一覧が表示される
- [ ] ロール・ステータス・法人でフィルタリングできる
- [ ] 氏名・メールアドレスで検索できる
- [ ] 検索結果の件数が表示される
- [ ] 各ユーザータイプに応じた編集ページへのリンクが機能する
- [ ] 管理者以外はアクセスできない
- [ ] ページネーション（将来実装）の準備ができている

## 注意事項
- 検索は大文字小文字を区別しない（ilike使用）
- 初期状態では全ユーザーを表示
- フィルター条件はURLパラメータで保持
- ページングは100件以上のユーザーがいる場合に実装
